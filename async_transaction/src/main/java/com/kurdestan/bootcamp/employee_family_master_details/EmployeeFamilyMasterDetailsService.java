...
public class EmployeeFamilyMasterDetailsService implements IEmployeeFamilyMasterDetailsService{
    ...
    public void save(Employee employee, List<Family> families) {
      ...
      Employee savedEmployee=  employeeService.saveWithTransaction(employee);
        int x=1/0;
        families.forEach(family -> family.setEmployee(savedEmployee));
        familyService.saveWithTransaction(families);
